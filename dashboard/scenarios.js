  ...
  const scenarios = [
    {
      ...
      baseAps: 4,
      nodes: [
        { id: "client", role: "client", icon: "💻", label: "정상 클라이언트", detail: "8C:F5:A3:11:02:44" },
        { id: "real-ap", role: "real-ap", icon: "📡", label: "진짜 AP", detail: "KT_GiGA_5G · CH 6" },
        { id: "attacker", role: "attacker", icon: "⌨️", label: "공격자", detail: "60:29:D5:03:9E:20" },
        { id: "sensor", role: "sensor", icon: "🔎", label: "WFSAT 센서", detail: "wlan1mon · CH 6" }
      ],
      defense: {
        name: "PMF (802.11w)",
        description: "관리 프레임에 무결성 보호를 적용하면 정상 AP가 보내지 않은 위조 Deauth 프레임을 클라이언트가 거부할 수 있습니다.",
        before: "연결 강제 해제",
        after: "위조 프레임 거부"
      },
      glossary: [
        { term: "Deauth", description: "연결을 종료하라고 알리는 인증 해제 관리 프레임입니다." },
      ...
      phases: [
        {
          ...
          description: "주기적인 Beacon과 데이터 프레임이 일정한 속도로 오가며 연결이 안정적으로 유지됩니다.",
          plain: "아직 공격은 없습니다. 센서가 정상적인 통신 패턴을 기준선으로 관찰하고 있습니다.",
          severity: "neutral", rate: 24, alerts: 0, evidence: ["Deauth 프레임이 거의 없음", "패킷 처리량이 정상 범위", "클라이언트 연결 상태가 안정적"],
          states: { client: "normal", "real-ap": "normal", attacker: "idle", sensor: "normal" },
          links: [link("real-ap", "client", "normal", "Beacon · Data"), link("real-ap", "sensor", "normal", "정상 관측")],
          packet: packet("real-ap", "client", "DATA", "normal"),
          event: event("Beacon", "normal", "60:29:D5:03:9E:20", "정상 AP Beacon 수신", "정상"),
          packetFields: { "Frame": "Management / Beacon", "Rate": "24 pkt/s", "Connection": "Associated" }
        },
        {
          label: "공격 준비", kicker: "RECONNAISSANCE", title: "공격자가 AP와 클라이언트 주소를 수집합니다",
          description: "공격자는 무선 채널을 관찰해 SSID, BSSID, 연결된 클라이언트 MAC 주소를 알아냅니다.",
          plain: "공격자가 누구인 척 위장할지 알아내기 위해 주변 통신을 엿보고 있습니다.",
          severity: "low", rate: 31, alerts: 0, evidence: ["동일 채널에서 Probe 요청 증가", "새로운 송신자 MAC 관측", "아직 직접적인 연결 방해는 없음"],
          states: { client: "normal", "real-ap": "normal", attacker: "warning", sensor: "normal" },
          links: [link("real-ap", "client", "normal", "정상 연결"), link("attacker", "real-ap", "warning", "채널 관찰"), link("attacker", "sensor", "warning", "Probe")],
          packet: packet("attacker", "real-ap", "PROBE", "warning"),
          event: event("Probe Request", "warning", "60:29:D5:03:9E:20", "타겟 SSID 탐색 요청", "관찰"),
          packetFields: { "Frame": "Management / Probe", "Channel": "6", "Target BSSID": "60:29:D5:03:9E:20" }
        },
        {
          label: "공격 진행", kicker: "FRAME INJECTION", title: "위조된 Deauth 프레임이 대량으로 주입됩니다",
          description: "공격자는 진짜 AP의 BSSID를 발신지로 위조하고 클라이언트에게 연결을 끊으라는 프레임을 반복 전송합니다.",
          plain: "공격자가 진짜 AP인 척하며 노트북에게 계속 ‘연결을 끊어’라고 명령하고 있습니다.",
          severity: "high", rate: 486, alerts: 38, evidence: ["2초 동안 Deauth 121개 수신", "reason code 7이 반복됨", "송신 속도와 sequence 흐름이 비정상"],
          states: { client: "warning", "real-ap": "normal", attacker: "danger", sensor: "warning" },
          links: [link("real-ap", "client", "normal", "정상 연결"), link("attacker", "client", "attack", "위조 Deauth"), link("attacker", "sensor", "warning", "비정상 급증")],
          packet: packet("attacker", "client", "DEAUTH", "attack"),
          event: event("Deauth", "attack", "60:29:D5:03:9E:20", "Reason 7 · 121 frames / 2s", "위험"),
          packetFields: { "Subtype": "0x0C (Deauthentication)", "Reason Code": "7", "Burst": "121 frames / 2s" }
        },
        {
          label: "영향 발생", kicker: "CONNECTION LOST", title: "정상 클라이언트의 연결이 끊어집니다",
          description: "클라이언트는 위조 프레임을 진짜 AP의 명령으로 오인하고 연결을 해제한 뒤 재접속을 반복합니다.",
          plain: "사용자는 와이파이가 계속 끊기고 다시 연결되는 현상을 경험합니다.",
          severity: "critical", rate: 682, alerts: 64, evidence: ["Association 상태가 Disconnected로 변경", "재연결 요청이 짧은 간격으로 반복", "패킷 손실과 응답 지연 급증"],
          states: { client: "offline", "real-ap": "warning", attacker: "danger", sensor: "danger" },
          links: [link("real-ap", "client", "inactive", "연결 끊김"), link("attacker", "client", "attack", "Deauth Flood"), link("sensor", "attacker", "warning", "공격 추적")],
          packet: packet("attacker", "client", "DEAUTH", "attack"),
          event: event("Connection Lost", "attack", "8C:F5:A3:11:02:44", "Association 상태 해제", "심각"),
          packetFields: { "Client State": "Disconnected", "Retry": "18 / 5s", "Packet Loss": "71%" }
        },
        {
          label: "탐지·대응", kicker: "DETECT & DEFEND", title: "센서가 공격을 분류하고 PMF 적용을 권고합니다",
          description: "WFSAT는 프레임 비율, 발신자, reason code를 함께 분석해 Deauth Flood로 판정하고 관리 프레임 보호를 안내합니다.",
          plain: "단순한 연결 오류가 아니라 누군가 위조 메시지를 반복 전송한 공격이라는 근거가 확인됐습니다.",
          severity: "resolved", rate: 96, alerts: 1, evidence: ["Deauth 비율이 기준선의 28배", "등록 AP와 송신 특성 불일치", "PMF 적용 후 위조 프레임 거부 확인"],
          states: { client: "defense", "real-ap": "defense", attacker: "danger", sensor: "defense" },
          links: [link("real-ap", "client", "defense", "PMF 보호 연결"), link("sensor", "attacker", "defense", "차단·경고")],
          packet: packet("real-ap", "client", "PMF", "defense"),
          event: event("Threat Detected", "defense", "WFSAT", "Deauthentication Flood 분류", "대응 완료"),
          packetFields: { "Detection": "Deauth Flood", "Confidence": "97%", "Mitigation": "PMF required" }
        }
      ]
    },
    {
